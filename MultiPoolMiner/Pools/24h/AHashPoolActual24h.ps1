...
if ($Payout_Currencies) {
    $APIStatusRequest | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {$APIStatusRequest.$_.hashrate -gt 0} | ForEach-Object {
        ...
        $Divisor = 1000000000 * [Double]$APIStatusRequest.$_.mbtc_mh_factor

        $Stat = Set-Stat -Name "$($Name)_$($Algorithm_Norm)_Profit" -Value ([Double]$APIStatusRequest.$_.actual_last24h / $Divisor) -Duration (New-TimeSpan -Days 1)
