...
if ($Payout_Currencies) {
    $APIStatusRequest | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {$APIStatusRequest.$_.hashrate -gt 0} | ForEach-Object {
        ...
        $Stat = Set-Stat -Name "$($Name)_$($Algorithm_Norm)_Profit" -Value ([Double]((($APIStatusRequest.$_.estimate_last24h, ([double]($APIStatusRequest.$_.actual_last24h) / 1000) | Measure-Object -Average).Average) / $Divisor)) -Duration (New-TimeSpan -Days 1)
