...
if ($Payout_Currencies) {
    if ($Config.Pools.$(Name).UseActualPoolPrice) {
        $PriceAttribute = "actual_last24h"
        $DivisorMultiplier = 1000000000
        $Name = "$($Name)Actual24h"
    } 
    else {
        $PriceAttribute = "estimate_last24h"
    ...
    $APIStatusRequest | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {$APIStatusRequest.$_.hashrate -gt 0} | ForEach-Object {
        ...
        $Divisor = $DivisorMultiplier * [Double]$APIStatusRequest.$_.mbtc_mh_factor

        if ((Get-Stat -Name "$($Name)_$($Algorithm_Norm)_Profit") -eq $null) {$Stat = Set-Stat -Name "$($Name)_$($Algorithm_Norm)_Profit" -Value ([Double]$APIStatusRequest.$_.$PriceAttribute / $Divisor) -Duration (New-TimeSpan -Days 1)}
