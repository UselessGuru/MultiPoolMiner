...
if ($Miner_Config.Commands) {$Commands = $Miner_Config.Commands}
    ...
    $Commands = [PSCustomObject[]]@(
        ...
        [PSCustomObject]@{MainAlgorithm = "ethash2gb";    SecondaryAlgorithm = "blake14r";  ; SecondaryIntensity = 0;  MinMemGB = 2; Vendor = @("NVIDIA"); Params = ""} #Ethash2Gb & Blake14r dual mining, auto dual solver and intensity
        [PSCustomObject]@{MainAlgorithm = "ethash3gb";    SecondaryAlgorithm = "blake14r";  ; SecondaryIntensity = 0;  MinMemGB = 3; Vendor = @("NVIDIA"); Params = ""} #Ethash3Gb & Blake14r dual mining, auto dual solver and intensity
        [PSCustomObject]@{MainAlgorithm = "ethash";       SecondaryAlgorithm = "blake14r";  ; SecondaryIntensity = 0;  MinMemGB = 4; Vendor = @("NVIDIA"); Params = ""} #Ethash & Blake14r dual mining, auto dual solver and intensity
    ...
    $Commands | ForEach-Object {$Main_Algorithm_Norm = Get-Algorithm $_.MainAlgorithm; $_} | Where-Object {$Pools.$Main_Algorithm_Norm.Host} | ForEach-Object {
        ...
        if ($Miner_Device = @($Device | Where-Object {$Vendor -contains $_.Vendor_ShortName -and ([math]::Round((10 * $_.OpenCL.GlobalMemSize / 1GB), 0) / 10) -ge $MinMemGB})) {
            ...
            if ($Secondary_Algorithm) {
                ...
                $WarmupTime = 120
                if ($Miner_Device.Vendor_ShortName -eq "NVIDIA") { #Dual mining only works with NVIDIA
                    if ($Config.UseDeviceNameForStatsFileNaming) {
                        ...
                        $Miner_Name = ((@($Name) + @($Miner_Device.Name | Sort-Object) + @("$Main_Algorithm_Norm$Secondary_Algorithm_Norm") + @("$(if ($_.SecondaryIntensity) {"$($_.SecondaryIntensity)"})") | Select-Object) -join '-') -replace "[-]{2,}", "-"
                    }
